        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {skills.length > 0 ? (
            skills.map((skillGroup, index) => (
              <div
                key={index}
                className="bg-gradient-card rounded-xl p-4 md:p-6 shadow-card border border-border hover:shadow-elevated transition-smooth"
              >
                ...
                <ul className="list space-y-3">
                  {skillGroup.items.map((skill, skillIndex) => {
                    ...
                    return (
                      <li
                        key={skillIndex}
                        className="text-muted-foreground hover:text-primary transition-smooth flex items-center gap-3"
                      >
                        {IconComponent ? (
